...
export const Route = createFileRoute("/lesson22")({ component: Lesson22 });

function Lesson22() {
  const { lang, completeLesson } = useApp();
  const lesson = getLesson("l22")!;

  return (
    <div className="space-y-4">
      <div className="rounded-3xl bg-gradient-to-br from-[oklch(0.65_0.22_25)] via-[oklch(0.6_0.24_15)] to-[oklch(0.55_0.26_5)] p-5 shadow-soft">
        <p className="text-xs font-bold uppercase tracking-widest text-white/80">
          {t(UI.chapters, lang)} VI ·{" "}
          {t(
            { kk: "Мемлекеттік рәміздер", ru: "Государственные символы", en: "State Symbols" },
            lang,
          )}
        </p>
